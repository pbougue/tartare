...
# python image
PYTHON_IMAGE = 'python:3.4-alpine'
TARTARE_CONTAINER_NAME = 'tartare_test'
SHARED_VOLUME = './test_dir'
...
def _get_docker_file():
    ...
    return BytesIO("FROM {}".format(PYTHON_IMAGE).encode())


class TartareTestDocker(object):
    """
    ...
    def __init__(self):
        ...
        for build_output in self.docker.build(fileobj=_get_docker_file(),
                                              tag=PYTHON_IMAGE, rm=True):
            log.debug(build_output)

        self.container_id = self.docker.create_container(PYTHON_IMAGE,
                                                         name=TARTARE_CONTAINER_NAME,
                                                         volumes=[SHARED_VOLUME]).get('Id')
    ...
    def __exit__(self, *args, **kwargs):
        ...
        for cont in self.docker.containers(all=True):
            if cont['Image'].split(':')[0] == PYTHON_IMAGE:
