    ...
    with app.app_context():
        ...
        for key, value in map_test.items():
            assert isinstance(PreProcessManager.get_preprocess(
                ContributorContext(Job(ACTION_TYPE_CONTRIBUTOR_EXPORT)), PreProcess(type=key)), value)
    # Coverage Preprocess
    for key in map_test.keys():
        with pytest.raises(InvalidArguments) as excinfo:
            PreProcessManager.get_preprocess(CoverageContext(Job(ACTION_TYPE_COVERAGE_EXPORT)),
                                             PreProcess(type=key))
...
def test_coverage_preprocess():
    ...
    # Coverage Preprocess
    for key, value in map_test.items():
        assert isinstance(PreProcessManager.get_preprocess(CoverageContext(Job(ACTION_TYPE_COVERAGE_EXPORT)),
                                                           PreProcess(type=key, params={'url': 'http://fusio.com'})),
                          value)
    # Contributor Preprocess
    for key in map_test.keys():
        with pytest.raises(InvalidArguments) as excinfo:
            PreProcessManager.get_preprocess(ContributorContext(Job(ACTION_TYPE_CONTRIBUTOR_EXPORT)),
                                             PreProcess(type=key))
        assert str(excinfo.typename) == "InvalidArguments"
...
def test_coverage_invalid_preprocess():
    with pytest.raises(InvalidArguments) as excinfo:
        isinstance(PreProcessManager.get_preprocess(CoverageContext(Job(ACTION_TYPE_COVERAGE_EXPORT)),
...
def test_contributor_invalid_preprocess():
    with pytest.raises(InvalidArguments) as excinfo:
        isinstance(PreProcessManager.get_preprocess(ContributorContext(Job(ACTION_TYPE_CONTRIBUTOR_EXPORT)),
                                                    PreProcess(type='AA')), FusioPreProd)
    assert str(excinfo.typename) == "InvalidArguments"
...
def test_launch_in_sequence(mock_run_preprocess):
    ...
    launch(preprocesses, ContributorContext(Job(ACTION_TYPE_CONTRIBUTOR_EXPORT)))
