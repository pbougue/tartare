...
def merge(coverage: Coverage, context: CoverageContext) -> CoverageContext:
...
def postprocess(coverage: Coverage, context: CoverageContext) -> CoverageContext:
    logger.info("postprocess for coverage_id = %s", coverage.id)
    # @TODO launch post process with tartare.tasks.launch function
    return context


def save_export(coverage: Coverage, context: CoverageContext) -> Optional[CoverageExport]:
    contributor_exports = []
    for contributor_context in context.contributor_contexts:
        data_sources = []
        for data_source_context in contributor_context.data_source_contexts:
