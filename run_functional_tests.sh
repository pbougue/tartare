...
docker build -t tartare_functional_tests -f docker/docker_file/Dockerfile .
docker build -t tartare_ruspell_functional_tests -f docker/docker_file/Dockerfile.ruspell.test .
docker-compose -f docker/docker_compose/docker-compose.test.yml up -d
HTTP_SERVER_ID=$(docker-compose -f docker/docker_compose/docker-compose.test.yml ps -q http_download_server)
